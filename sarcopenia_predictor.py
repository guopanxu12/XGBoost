...
if st.button("Predict (预测)"):
    ...
    if predicted_class == 1:
        advice = (
            f"根据模型预测，您可能存在较高的肌少症发病风险。 (According to the model, you may have a high risk of developing sarcopenia.)\n"
            f"模型预测的发病概率为 {probability:.1f}%。 (The predicted probability of developing sarcopenia is {probability:.1f}%)\n"
            "建议您尽快就医，以进行更详细的诊断和采取适当的治疗措施。 (It is recommended that you see a doctor as soon as possible for a more detailed diagnosis and appropriate treatment.)"
        )
    else:
        advice = (
            f"根据模型预测，您的肌少症风险较低。 (According to the model, your risk of sarcopenia is low.)\n"
            f"模型预测的无肌少症概率为 {probability:.1f}%。 (The predicted probability of not having sarcopenia is {probability:.1f}%)\n"
            "建议您继续保持健康的生活方式，并定期观察健康状况。如有任何异常症状，请及时就医。 (It is recommended that you maintain a healthy lifestyle and monitor your health regularly. If you experience any symptoms, please see a doctor promptly.)"
        )
